<script lang="ts">
    ...
    let hasActiveFilters = $derived(
        ...
        activeSchedules.length > 0 || activeModalities.length > 0
    );
                    ...
                    {#if table.getRowModel().rows.length === 0}
                        <Table.Row class="hover:bg-transparent">
                            <Table.Cell colspan={8} class="h-[350px] text-center">
                                <div class="flex flex-col items-center justify-center text-center p-8 animate-in fade-in-50 zoom-in-[0.98] duration-300">
                                    <div class="flex h-20 w-20 items-center justify-center rounded-full bg-muted/40 border border-border/50 mb-5 shadow-sm">
                                        {#if hasActiveFilters}
                                            <Search size={32} class="text-muted-foreground/50" strokeWidth={1.5} />
                                        {:else}
                                            <CalendarX2 size={32} class="text-muted-foreground/50" strokeWidth={1.5} />
                                        {/if}
                                    </div>
                                    <h3 class="text-lg font-bold text-foreground tracking-tight mb-1.5">
                                        {hasActiveFilters ? "No matches found" : "No sections configured"}
                                    </h3>
                                    <p class="text-sm font-medium text-muted-foreground max-w-md mx-auto leading-relaxed text-center text-balance">
                                        {hasActiveFilters
                                            ? "We couldn't find any sections matching your current filters. Try adjusting them."
                                            : "Click \"Add Section\" above to start populating this course's available schedules."}
                                    </p>
                                    {#if hasActiveFilters}
                                        <Button variant="outline" size="sm" class="mt-5 gap-2 shadow-sm font-semibold" onclick={clearFilters}>
